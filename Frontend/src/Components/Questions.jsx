...
function generateQuestionId() { return questionIdCounter++; }
function generateOptionId() { return optionIdCounter++; }
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl px-7 pt-6 pb-5 shadow-sm transition-shadow duration-200 hover:shadow-md">
      ...
      <input
        ...
        className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg outline-none text-gray-800 placeholder-gray-300 focus:border-gray-400 focus:ring-2 focus:ring-gray-100 transition-all duration-150 mb-5"
      />
      ...
      <div className="flex mt-4 cursor-pointer items-center gap-3">
          <button className="btn-primary cursor-pointer flex-1 rounded-xl bg-black py-3.5 text-sm font-semibold text-white">
            Add Question
          </button>
        </div>
    </div>
...
export default function FormBuilder() {
  ...
  const handleRemoveQuestion = (id) =>
    setQuestions((prev) =>
      prev.filter((q) => q.id !== id).map((q, i) => ({ ...q, index: i + 1 }))
    );
